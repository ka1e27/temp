...
export function playerTurn(state, opts = {}) {
  ...
  // ...AND THE ONE THING IT DOES BECAUSE SOMETHING IS BEING DONE TO IT: march
  // home to meet the enemy's set-piece. Goes FIRST, and its sources join
  // `usedSources`. Whole argument, and the proof that it is inert without a
  ...
  for (const src of mine) {
    // Spent this think — today only by `defendTurn`, since this loop is the
    // only other writer and visits each site once, so this is inert for every
    // measured number. A garrison recalled home and then marched at a farm on
    // the same think would be neither.
    if (usedSources.has(src.id)) continue;
