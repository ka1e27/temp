...
test('the contract carries shop features, and validates them', () => {
  assert.equal(CONTRACT_VERSION, 14,
    ...
    + ' v10 is the same shape a THIRD time — a squad gained the `path` it walks,'
    + ' a nullable `to` (a march onto bare ground) and a `camped` hex, so a v9'
    + ' squad has no route to read a position off and every column in the air is'
    + ' invisible to the renderer, to fog and to the towers that shoot at'
    + ' positions, while still arriving on schedule; v11 adds one real field,'
    + ' `lastKnownGarrison`, alongside two behaviour changes (squad sight,'
    + ' watchtower counter-intelligence) that need no field at all because both'
    + ' read only what v10 already crosses the seam with; v12 is the v8 shape a'
    ...
    + ' v13 is the v8 shape a FIFTH time — `state.ai` gained `musterTick`,'
    ...
    + ' and raises a SECOND one; and v14 is the SIXTH — the set-piece became a'
    + ' schedule of three escalating waves, so `state.ai` gained `musterWave` and'
    + ' a v13 blob reads the missing index as 0, which re-opens wave ONE at a'
    + ' tick still inside its own window and raises the host it already spent.'
    + ' The ENEMY_MUSTER event beside either needed no bump of its own: nothing'
    + ' in battle/ or tools/ reads state.events at all');
