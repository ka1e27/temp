...
test('harness: --noconstruct really is the bot that could not build', () => {
  ...
  //
  // `scout: false` alongside it: `scoutTurn` is a SEPARATE mechanic behind its
  // OWN flag (see the pair of tests below), not a second thing this one gates.
  const battle = startRun('karrowmere', 1000, before('karrowmere'), 10);
  let nextThink = 0;
  let built = 0;
  while (battle.status === 'running' && battle.tick < 4800) {
    if (battle.tick >= nextThink) {
      playerTurn(battle, { construct: false, scout: false });
      nextThink = battle.tick + 20;
    }
